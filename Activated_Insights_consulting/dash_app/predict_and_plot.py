...
def main():
    ...
    predictions = [predict_new_labels(models, embeddings) for model in models]
...
def load_models():
    ...
    model_path = '/home/matt_valley/PycharmProjects/insight_2020a_project/Activated_Insights_consulting/tri_train_models_disagree.pkl'
...
def load_embeddings(path='/home/matt_valley/PycharmProjects/insight_2020a_project/Activated_Insights_consulting/hand_labelled_bert_embeddings.npy'):

    embeddings = np.load(path)
    print(embeddings.shape)
    embeddings = tri_training.pca_reduce(embeddings)
    print(embeddings.shape)
...
def get_class_frequency(df):
    classes = get_classes()
    class_counts = [np.sum(df[c]) for c in classes]
